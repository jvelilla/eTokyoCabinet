...
feature -- Test routines
	test_clear
		do
			assert ("Empty Map", map_string.records_number = 0)
			assert ("Empty Map", map_string.is_empty)
			map_string.put ("key1", "value1")
			map_string.put ("key2", "value2")
			assert ("two records", map_string.records_number = 2)
	...
	test_remove_string
		do
			...
			assert ("one records", map_string.records_number = 1)
		end
	...
	test_remove
		...
		local
			p : PERSON
		do
			assert ("Empty Map", map.records_number = 0)
			create p.make ("javier", "velilla")
			map.put (p, "value")
			create p.make ("manuel", "ginobili")
			map.put (p, "value2")
			assert ("Map with two element", map.records_number = 2)
			assert ("Expected value", map.get (p).is_equal ("value2"))
			map.remove (p)
			assert ("Map with one element", map.records_number = 1)
		end
	test_put_cat_string
		do
			assert ("Empty Map", map_string.records_number = 0)
	...
	test_put_string
		...
		do
			...
			assert ("Map with one element", map_string.records_number = 1)
	...
	test_keys
		...
		do
			...
			map.put (p, "value")
			assert ("Map with one element", map.records_number = 2)
			assert ("Expected value", map.get (p).is_equal ("value"))
			l_list := map.map_keys
	...
	test_keys_string
		...
		do
			...
			assert ("Map with for elements", map_string.records_number = 4)
...
feature -- Implementation
	map_string : TC_MAP_API [STRING,STRING]
	map        : TC_MAP_API [STRING,PERSON]
